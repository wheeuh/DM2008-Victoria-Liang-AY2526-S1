...
function draw() {
  background(100);
  fill(circleSize, 255, 255);
  ellipse(width / 5 - circleSize, height / 2, circleSize);
  fill(circleSize +30 , 255, 255);
  ellipse(width / 5 * 2 - circleSize, height / 2, circleSize);
  fill(circleSize +60, 255, 255);
  ellipse(width / 5 * 3 - circleSize, height / 2, circleSize);
  fill(circleSize +90, 255, 255);
  ellipse(width / 5 * 4 - circleSize, height / 2, circleSize);
  fill(circleSize +120, 255, 255);
  ellipse(width / 5 * 5 - circleSize, height / 2, circleSize);
  ...
  if (port.opened()) {
    ...
    if (sensorVal[0]) {
      ...
      //the sensorVal comes in as a string so we need to convert it to a float (decimal)
      // last value in lerp() controls speed of change
      // lerp vs opt1 is js, instead of opt1: when dist 500 make circle 500, opt 2: when 500 ease to 500
